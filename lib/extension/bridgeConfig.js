    ...
    async devices(topic, message) {
        ...
        const devices = this.zigbee.getDevices().map((device) => {
            ...
            if (device.type !== 'Coordinator') {
                ...
                if (friendlyDevice) {
                    const entity = settings.getEntity(friendlyDevice.friendly_name);
                    assert(entity, `Entity '${friendlyDevice.friendly_name}' does not exist`);
                    const options = settings.getDeviceOptions(entity.ID)
